...
export default function Testimonial() {
    ...
    return (
      <section className="bg-white py-8 px-5 m-auto max-w-screen-xl text-center">
        <h2 className="mb-4 text-4xl font-extrabold text-black dark:text-black">Testimonials</h2>
        <p className="mb-8 text-gray-500 font-semibold dark:text-orange-400">Our Happy Clients</p>
        <div className="grid gap-8 lg:grid-cols-2">
          {testimonials.map(({ name, title, image, quote }, index) => (
            <div
              key={index}
              className="flex flex-col relative justify-between items-center p-6 border rounded-lg shadow-md bg-primary border-darkorange dark:shadow-lg"
            >
              <img src={quoteimg} className='absolute h-14 w-auto -left-1 -top-6' alt="" />
              <blockquote className="mb-4 text-gray-500 dark:text-gray-400">
                <h3 className="text-lg font-semibold text-darkorange">{quote.split(" ")[0]}</h3>
                <p className="my-4 text-justify leading-relaxed text-black">{quote}</p>
              </blockquote>
              <figcaption className="flex justify-center items-center space-x-3">
                <img className="w-9 h-9 rounded-full" src={image} alt={`${name} profile`} />
                <div className="text-left text-sm font-light text-black">
                  <div>{name}</div>
