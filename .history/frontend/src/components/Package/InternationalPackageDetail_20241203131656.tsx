...
const InternationalPackageDetail = () => {
    ...
    useEffect(() => {
        ...
        if (id) {
            axios.get<InternationalPackage>(`http://192.168.1.86:5000/api/internationalpackage/${id}`)
                .then(response => {
    ...
    return (
        <div>
            <div className='relative'>
                ...
                <h1 className='text-xl md:text-5xl font-extrabold'>{internationalpackage.package_name}</h1>
                <div className='flex items-center gap-2 pt-2 pb-10'>
                    <Link to='/internationalpackages' className='hover:text-darkorange duration-300 transition whitespace-nowrap'>International Packages</Link>
                    <FontAwesomeIcon icon={faChevronRight} />
                    {internationalpackage.package_name}
                    </div>
                </div>
            </div>
            <div className="container mx-auto px-5 py-10 lg:px-24">
