...
export default function Home() {
  ...
  useEffect(() => {
    ...
    axios.get<Activity[]>('http://192.168.1.86:5000/api/activities')
      .then(response => {
  ...
  useEffect(() => {
    // Fetch the activities from the backend
    axios.get<InternationalPackage[]>('http://192.168.1.86:5000/api/internationalpackages')
      .then(response => {
  ...
  useEffect(() => {
    // Fetch the activities from the backend
    axios.get<NepalPackage[]>('http://192.168.1.86:5000/api/nepalpackages')
      .then(response => {
      ...
      <section className="text-gray-600 body-font">
        <div className="md:flex justify-between items-center px-5 py-10 md:px-24">
          ...
          <Link to='/activities' onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}>
            <button className="inline-flex items-center text-primary bg-darkorange border px-4 py-0 border-darkorange focus:outline-none hover:bg-primary text-lg hover:text-darkorange rounded-3xl h-10 md:mt-0 transition duration-300 my-2" onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}>
              See All Packages
      ...
      <div className='bg-primary'>
        <section className="text-gray-600 body-font">
          <div className="md:flex justify-between items-center px-5 py-10 md:px-24">
            ...
            <Link to='/internationalpackages' onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}>
          ...
          <div className="container px-5 pb-10 mx-auto md:px-24">
            <div className="grid gap-6 grid-cols-1 sm:grid-cols-2 md:grid-cols-3">
              {displayedInternationalPackages.map((internationalpackage, id) => (
                <Link to={`/internationalpackage/${internationalpackage.id}`} onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}>
                  <div
                    key={id}
...
{/* Nepal Packages */}
      <section className="relative min-h-screen w-full bg-cover bg-center" style={{ backgroundImage: `url(${homebg})` }}>
        {/* Overlay to make the background darker */}
        <div className="absolute inset-0 bg-black bg-opacity-40"></div>

        {/* Your content here */}
        <div className="relative z-10 text-white h-fit min-h-full mx-auto p-5 px-5 py-10 md:px-24">
          <h1 className="md:text-8xl text-4xl font-bold font-fira md:mt-20">EXPLORE NEPAL</h1>
          <hr className='mt-20 mb-10' />
          <div className='flex justify-between flex-col md:flex-row'>
            <div className=''>
              <p className='max-w-96'>Choose us for your Nepal tour for a personalized, seamless experience. With expert guides and custom itineraries, we ensure you explore Nepal's stunning landscapes, rich culture, and heritage with ease and comfort. Let us make your adventure unforgettable.</p>
              <Link to='/nepalpackages' onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}>
                <button className="inline-flex items-center text-black bg-white border px-4 py-0 border-white focus:outline-none hover:bg-transparent text-lg hover:text-white rounded-xl h-10 my-10 transition duration-300">
              ...
              {displayedPackages.map((nepalpackage, id) => (
                <div key={id}>
                  <Link to={`/nepalpackage/${nepalpackage.id}`} onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}>
                    <img src={nepalpackage.imageurl} alt="" className='h-[15rem] object-cover w-48' />
                    <h3>{nepalpackage.place}</h3>
                  </Link>
                </div>
              ))}
            </div>
