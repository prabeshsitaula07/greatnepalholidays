      ...
      <div className="container px-5 pb-10 mx-auto md:px-24">
        <div className="grid gap-6 grid-cols-1 sm:grid-cols-2 md:grid-cols-4">
          {paymentpartners.map((paymentpartner, index) => (
              <img key={index} src={paymentpartner.imgSrc} alt={paymentpartner.alt} className='hover:cursor-pointer transform transition duration-700 ease-in-out hover:scale-105' />
          ))}
        </div>
        </div>
    </section>
