        ...
        <div className="grid gap-6 grid-cols-1 sm:grid-cols-2 md:grid-cols-4">
          {paymentpartners.map((paymentpartner, index) => (
            <div key={index} className="hover:cursor-pointer transform transition duration-700 ease-in-out hover:scale-105 hover:shadow-lg bg-primary">
              <img src={paymentpartner.imgSrc} alt="" />
            </div>
          ))}
        </div>
        <p className='italic pt-7 text-xl text-center text-gray-500'>"Travel is not just about the destination,<br />it's about the journey we 
      </div>
    </section>
  );
}
