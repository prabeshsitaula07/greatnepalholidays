...
export default function TripHandling() {
  ...
  return (
    <div className="bg-primary h-fit flex justify-center flex-col items-center py-10 px-5 md:px-24">
      <h2 className="mb-10 text-4xl tracking-tight font-extrabold text-darkorange text-center font-parkinsans">
        We Will Handle Trip For You
      </h2>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 w-full">
        {services.map((service, index) => (
          <div key={index} className="flex gap-10 items-center">
            <img className="h-40 w-40 object-cover" src={service.imgSrc} alt={service.title} />
            <div>
              <h3 className="font-semibold pb-2 text-2xl text-black">{service.title}</h3>
              <p className="font-parkinsans text-black">{service.description}</p>
            </div>
          </div>
        ))}
      </div>
