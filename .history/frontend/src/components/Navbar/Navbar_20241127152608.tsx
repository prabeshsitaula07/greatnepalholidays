...
export default function Navbar() {
    ...
    // Separate states for "Activities" and "Nepal Packages" dropdown visibility
    const [isActivitiesDropdownVisible, setIsActivitiesDropdownVisible] = useState(false);
    const [isNepalPackagesDropdownVisible, setIsNepalPackagesDropdownVisible] = useState(false);
    ...
    useEffect(() => {
        // Fetch Nepal packages from the backend
        axios.get<NepalPackage[]>('http://localhost:5000/api/nepalpackages')
            .then(response => {
                setNepalPackages(response.data);
            })
            .catch(error => {
                console.error('There was an error fetching the Nepal packages!', error);
            });
    }, []);
    ...
    const handleMouseEnter = (menu: string) => {
        ...
        } else if (menu === 'nepalpackages') {
            setIsNepalPackagesDropdownVisible(true);
        }
    ...
    const handleMouseLeave = (menu: string) => {
        const newTimeoutId = setTimeout(() => {
            if (menu === 'activities') {
                ...
                setIsNepalPackagesDropdownVisible(false);
            }
        }, 300);
    ...
    const handleListMouseLeave = (menu: string) => {
        const newTimeoutId = setTimeout(() => {
            if (menu === 'activities') {
                setIsActivitiesDropdownVisible(false);
            } else if (menu === 'nepalpackages') {
            ...
            >
                <div className="container mx-auto flex flex-wrap p-5 md:px-24 flex-col md:flex-row items-center">
                    ...
                    <nav className="md:ml-auto flex flex-wrap items-center justify-center text-base">
                        ...
                        <Link to='' className="mr-5 hover:text-darkorange transition-all duration-200 ease-in-out">International Packages</Link>
                        <div
                            className="relative mr-5"
                            onMouseEnter={() => handleMouseEnter('activities')}
                            onMouseLeave={() => handleMouseLeave('activities')}
                        >
                            <Link to="#" className="hover:text-darkorange transition-all duration-200 ease-in-out">
                                Activities <FaCaretDown className="inline text-xl font-light" />
                            </Link>
                            {isActivitiesDropdownVisible && (
                                <div
                        ...
                        >
                            ...
                            {isNepalPackagesDropdownVisible && (
                                <div
                                    className="absolute left-0 top-full bg-primary text-black shadow-md p-4 z-30 mt-8 whitespace-nowrap w-fit transition-all duration-200 ease-in-out"
                                    onMouseEnter={handleListMouseEnter}
                                    onMouseLeave={() => handleListMouseLeave('nepalpackages')}
                                >
                                    <ul>
                                        {nepalpackages.map((nepalpackage, id) => (
                                            <li key={id} className="hover:text-darkorange py-1 transition-all duration-200 ease-in-out">
                                                <Link to={`/activity/${nepalpackage.id}`}>{nepalpackage.package_name}</Link>
                                            </li>
                                        ))}
                                    </ul>
                                </div>
