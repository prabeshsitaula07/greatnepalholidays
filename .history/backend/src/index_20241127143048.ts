...
app.post('/api/add-activity', (req: Request, res: Response) => {
  ...
  const query = `INSERT INTO activity (activity, place, price, country, image_url, description)
                 VALUES (?, ?, ?, ?, ?, ?)`;
...
app.post('/api/add-nepalpackage', (req: Request, res: Response) => {
  const { nepalpackage, place, price, country, image_url, description } = req.body;

  // Type assertion to ensure price is a number (since it comes as a string in the request body)
  const query = `INSERT INTO nepalpackages (package_name, place, price, country, image_url, description)
                 VALUES (?, ?, ?, ?, ?, ?)`;
