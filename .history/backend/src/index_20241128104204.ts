...
app.post('/api/add-nepalpackage', (req: Request, res: Response) => {
  const { nepalpackage, place, price, country, imageurl, description } = req.body;

  // Type assertion to ensure price is a number (since it comes as a string in the request body)
  const query = `INSERT INTO nepalpackages (package_name, place, price, country, imageurl, description)
                 VALUES (?, ?, ?, ?, ?, ?)`;

  db.query(query, [nepalpackage, place, parseFloat(price), country, imageurl, description], (err, result: mysql.ResultSetHeader) => {
    if (err) {
      return res.status(500).json({ error: 'Failed to add activity' });
    }
    // Use result.insertId when result is of type ResultSetHeader
...
app.post('/api/add-internationalpackage', (req: Request, res: Response) => {
  const { internationalpackage, place, price, country, image_url, description } = req.body;

  // Type assertion to ensure price is a number (since it comes as a string in the request body)
  const query = `INSERT INTO internationalpackages (package_name, place, price, country, image_url, description)
                 VALUES (?, ?, ?, ?, ?, ?)`;

  db.query(query, [internationalpackagpackage, place, parseFloat(price), country, image_url, description], (err, result: mysql.ResultSetHeader) => {
    if (err) {
      return res.status(500).json({ error: 'Failed to add activity' });
    }
    // Use result.insertId when result is of type ResultSetHeader
