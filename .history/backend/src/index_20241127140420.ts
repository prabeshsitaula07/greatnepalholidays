...
app.post('/api/add-nepalpackage', (req: Request, res: Response) => {
  const { package, place, price, country, imageurl, description } = req.body;

  // Type assertion to ensure price is a number (since it comes as a string in the request body)
  const query = `INSERT INTO nepalpackages (package_name, place, price, country, imageurl, description)
                 VALUES (?, ?, ?, ?, ?, ?)`;

  db.query(query, [package, place, parseFloat(price), country, imageurl, description], (err, result: mysql.ResultSetHeader) => {
    if (err) {
...
app.get('/api/nepalpackage/:id', (req: Request, res: Response) => {
  ...
  const query = 'SELECT * FROM nepalpackage WHERE id = ?';
  ...
  db.query(query, [packageId], (err, results) => {
    ...
    // Assert that `results` is an array of objects (or records)
    const package_name = (results as any[])[0]; // Cast the results to any[] and access the first element
    
    if (!package_name) {
      return res.status(404).json({ error: 'Package not found' });
    }
    
    res.status(200).json(package_name);
  });
});
